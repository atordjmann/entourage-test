...
import { MovieDetailDto, MovieDto, UpcomingMoviesDto } from "./movies";
...
export const moviesApiSlice = createApi({
  ...
  tagTypes: ["Movies"],
  endpoints: (build) => ({
    getUpcomingMovies: build.query<UpcomingMoviesDto, number>({
    ...
    getSimilarMoviesById: build.query<UpcomingMoviesDto, number>({
      query: (id) => `/${id}/similar`,
      providesTags: (result, error, id) => [{ type: "Movies", id }],
      serializeQueryArgs: ({ endpointName }) => {
        return endpointName
...
export const { useGetUpcomingMoviesQuery, useGetByIdQuery, useGetSimilarMoviesByIdQuery } = moviesApiSlice;